# This Makefile is part of Marten web server project,
# https://github.com/daokoder/marten
#
...
ALL_SOURCES = main.c marten.c build/sqlite3.c build/lsqlite3.c \
...
MARTEN_FLAGS = -DNO_SSL_DL -DUSE_LUA -DUSE_LUA_SQLITE3 $(COPT)

FLAGS = $(MARTEN_FLAGS) $(SQLITE_FLAGS) $(YASSL_FLAGS) $(LUA_FLAGS)

# Using Visual Studio 6.0. To build Marten:
...
linux:
	$(CC) marten.c main.c -o $(PROG) -ldl $(CFLAGS)

mac: bsd
bsd:
	$(CC) marten.c main.c -o $(PROG) $(CFLAGS)

bsd_lua: $(ALL_OBJECTS)
	$(CC) $(ALL_OBJECTS) -o $@

solaris:
	$(CC) marten.c main.c -lnsl -lsocket -o $(PROG) $(CFLAGS)
...
Marten: marten.c main.c
	$(CC) marten.c main.c build/lsqlite3.c build/sqlite3.c \
          -DUSE_COCOA $(CFLAGS) $(FLAGS) -mmacosx-version-min=10.4 \
          $(YASSL_SOURCES) $(LUA_SOURCES) \
          -framework Cocoa -ObjC -arch i386 -arch x86_64 -o Marten

cocoa: Marten
	V=`perl -lne '/define\s+MARTEN_VERSION\s+"(\S+)"/ and print $$1' marten.c`; DIR=dmg/Marten.app && rm -rf $$DIR && mkdir -p $$DIR/Contents/{MacOS,Resources} && install -m 644 build/marten_*.png $$DIR/Contents/Resources/ && install -m 644 build/Info.plist $$DIR/Contents/ && install -m 755 Marten $$DIR/Contents/MacOS/ && ln -fs /Applications dmg/ ; hdiutil create Marten_$$V.dmg -volname "Marten $$V" -srcfolder dmg -ov #; rm -rf dmg
...
mingw:
	...
	$(CC) $(MINGWOPT) marten.c -lws2_32 \
		-shared -Wl,--out-implib=$(PROG).lib -o $(PROG).dll
	$(CC) $(MINGWOPT) marten.c main.c build\res.o \
...
cygwin:
	windres ./build/res.rc ./build/res.o
	$(CC) $(CYGWINOPT) marten.c -lws2_32 \
		-shared -Wl,--out-implib=$(PROG).lib -o $(PROG).dll
	$(CC) $(CYGWINOPT) -Ibuild marten.c main.c ./build/res.o \
...
tarball: clean
	F=marten-`perl -lne '/define\s+MARTEN_VERSION\s+"(\S+)"/ and print $$1' marten.c`.tgz ; cd .. && tar -czf x marten/{LICENSE,Makefile,examples,test,build,*.[ch],*.md} && mv x marten/$$F

release: tarball cocoa
	wine make windows
	V=`perl -lne '/define\s+MARTEN_VERSION\s+"(\S+)"/ and print $$1' marten.c`; upx marten.exe; cp marten.exe marten-$$V.exe; cp marten.exe marten_php_bundle/; zip -r marten_php_bundle_$$V.zip marten_php_bundle/
